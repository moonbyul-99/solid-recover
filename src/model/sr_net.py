...
class feature_encoder(nn.Module):
    '''
    generate the cell embedding using multiple MLP, not VAE encoder

    First, feature to feature program 
    then feature_program to cell_embedding 

    feature --Linear, LayerNorm1d--> feature program 

    feature program --> multi full connection net --> cell embedding 

    Used for logp(RNA), logp(ATAC), logp(gene activity), TF-IDF(ATAC) # continuous feature
    '''
    def __init__(self,
                 feature_num,
                 hidden_dims,
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            if i == 0:
                self.encoder_header = nn.Linear(feature_num, hidden_dim)
                self.layernorm = nn.LayerNorm(normalized_shape = hidden_dim,
                                                eps = layernorm_eps)
            else:
                fc_layers.append(fc_net(input_dim = hidden_dims[i-1],
    ...
    def forward(self,x):
        feature_program = self.encoder_header(x)
        feature_program = self.layernorm(feature_program)
        cell_embed = self.fc_layers(feature_program) 
        return cell_embed 

class feature_decoder(nn.Module):
    '''
    generate the raw feature from cell embedding using multiple MLP, not VAE endecoder

    First, cell embedding to feature program
    then feature_program to raw feature

    cell embedding  --> multi full connection net --> feature program

    feature program--Linear--> raw_feature

    Used for logp(RNA), logp(ATAC), logp(gene activity), TF-IDF(ATAC) # non-negative continuous feature
    '''
    def __init__(self,
                 feature_num,
                 hidden_dims,
        ...
        feature_num: int, the number of genes
        hidden_dims: list of int, the number of hidden dim, h[0] --> h[1] -->h[-1] --> feature num
        dropout: dropout rate, float
        activation: key of activateion, str in ('relu', 'rrelu','sigmoid','gelu','leaky_relu','tanh')
        layernorm_eps: float, the eps of layer norm
        ...
        for i,hidden_dim in enumerate(hidden_dims):
            if i == len(hidden_dims) - 1:
                self.decoder_header = nn.Linear(hidden_dims[-1], feature_num)
            else:
                fc_layers.append(fc_net(input_dim = hidden_dim,
    ...
    def forward(self,x):
        feature_program = self.fc_layers(x) 
        feature = self.decoder_header(feature_program)
        return feature 
...
class CLIPLoss(nn.Module):
    ...
    def forward(self, cell_1, cell_2):
        ...
        #loss_12 = F.cross_entropy(logits_cell_1, ground_truth, reduction = 'sum')
        
        # 计算文本到图像的损失
        loss_21 = F.cross_entropy(logits_cell_2, ground_truth)
        #loss_21 = F.cross_entropy(logits_cell_2, ground_truth, reduction= 'sum')
...
class multi_model(nn.Module):
    ...
    def freeze_param(self):
        ...
        for param in self.rna_ae.decoder_net.decoder_header.parameters():
            param.requires_grad = False
            
        for param in self.ga_ae.encoder_net.encoder_header.parameters():
            param.requires_grad = False
        for param in self.ga_ae.decoder_net.decoder_header.parameters():
            param.requires_grad = False
    ...
    def forward(self, rna, ga):
        ...
        loss = rna_loss + ga_loss + self.tau * clip_loss
